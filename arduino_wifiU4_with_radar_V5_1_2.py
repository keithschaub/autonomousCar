...
posX_value = 0
posY_value = 0
heading_value = 90 # initialize to 90
...
data_file = open("arduino_data_V5_1_1.txt", "w")
...
def update_heading(line):
    ...
    if len(parts) >= 2:
        # Attempt to remove any trailing commas and convert to float
        heading_str = parts[1].replace(',', '')  # Remove comma if present
        try:
            heading_value = float(heading_str)  # Convert to float
