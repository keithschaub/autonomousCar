...
data_file = open("arduino_data_V4_3.txt", "w")
...
def receive_data():
    global s, running, data_file, posX_value, posY_value  # Include data_file in global declaration
        ...
        try:
            ...
            for line in data_lines:
                ...
                if "posX" in line:
                    ...
                    for part in parts:
                        if "posX" in part:
                           posX_value = part.split()[1]
                        elif "posY" in part:
                            posY_value = part.split()[1]
...
def update_position_display():
    global ax, posX_value, posY_value, pos_text
    # Update position display
    if posX_value is not None and posY_value is not None:
        pos_str = f"PosX: {posX_value}, PosY: {posY_value}"
        if pos_text is None:
            pos_text = ax.text(0.7, 1.1, '', fontsize=10, transform=ax.transAxes)
        pos_text.set_text(pos_str)

def handle_heading(heading_line):
    # Extract heading value from the line
    global heading_value, heading_text
    heading_value = heading_line.split()[1]
    update_heading_display()
    #print(f'Heading: {heading_value} deg')

def update_heading_display():
    global ax, heading_value, heading_text
    # Update heading display
    if heading_value is not None:
        if heading_text is None:
            heading_text = ax.text(0.15, 1.1, '', fontsize=10, transform=ax.transAxes)
        heading_text.set_text(f"Heading: {heading_value} deg")

def update_radar():
    global ax, running, heading_value, heading_text, last_displayed_heading, pos_text, posX_value, posY_value
    ...
    while running:
        ...
        if count >= 50:
            ...
            # Reinitialize the heading and position text objects after clearing the plot
            heading_text = None
            pos_text = None

        # Update heading and position display outside the if not data_queue.empty() block
        if heading_value is not None:
            update_heading_display()
        if posX_value is not None and posY_value is not None:
            update_position_display()
