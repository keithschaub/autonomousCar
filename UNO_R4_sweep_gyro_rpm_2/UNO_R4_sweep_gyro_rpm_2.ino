...
void slowForward() {
 ...
 // Calculate RPM for Motors and distance traveled 
  ...
  String rpm_packet = "RPM1 " + String(rpm1) + "," + "RPM2 " + String(rpm2) + ", Counter1 " + String(counter1) + ", Counter2 " + String(counter2) + ", distanceTraveled " + String(distanceTraveled) + "\n";
...
void updatePosition(float totalDistance) {
    float averageHeading = (heading_start + heading_final) / 2;
    posX += totalDistance * cos(averageHeading * DEG_TO_RAD);
    posY += totalDistance * sin(averageHeading * DEG_TO_RAD);
}

float calculateDistanceMoved() {
  // add code here to calculate how far the car moved
}
